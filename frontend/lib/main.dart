...
class _HomePageState extends State<HomePage> {
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            ...
            builder: (context) => LobbyPage(
              ...
              socket: _socket,
              initialPlayers: [
                {'id': _socket.id, 'name': _nameController.text}
              ],
            ),
          ),
        ).then((_) {
  ...
  void _joinRoom() {
    if (_nameController.text.isEmpty || _roomCodeController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content:
                Text('Por favor, introduce tu nombre y el código de la sala')),
      );
      return;
    }
    setState(() => _isLoading = true);
  ...
  Widget _buildGradientTitle() {
    // Colores Azulgrana (oscuros para el tema)
    final Color azul = Colors.blue[800]!;
    final Color grana = Colors.red[900]!;
    ...
    return ShaderMask(
      ...
      child: const Text(
        ...
        style: TextStyle(
          ...
          color:
              Colors.white, // El color base (será enmascarado por el degradado)
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      // 2. QUITAMOS EL APPBAR
      appBar: null,

      body: Center(
        child: SingleChildScrollView(
          child: Padding(
            // 3. CORREGIMOS EL PADDING (200 era demasiado)
            padding: const EdgeInsets.symmetric(horizontal: 24.0),
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: <Widget>[
                // 4. AÑADIMOS EL ESPACIO SUPERIOR (PARA BAJAR EL TÍTULO)
                const SizedBox(height: 10), // Espacio desde el borde superior

                // 5. AÑADIMOS NUESTRO NUEVO TÍTULO
                _buildGradientTitle(),

                // 6. MENOS ESPACIO ENTRE TÍTULO E IMAGEN
                const SizedBox(height: 60),

                Image.asset(
                  'assets/images/logo_futbol.png',
                  width: 250,
                  height: 250,
                ),

                // 7. MÁS ESPACIO ENTRE IMAGEN Y TEXTFIELD
